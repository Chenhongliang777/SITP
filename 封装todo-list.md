...
| 3.4 | 静态 H5 | 设置、登录状态、任务表单、进度、日志（轮询）、报告链接 | ✅ |
| 3.5 | 报告访问 | 按 keyword 取最新 HTML + 令牌鉴权（防任意路径） | ✅ |
| 3.6 | 绑定与发现 | 默认 `0.0.0.0:8765`；GUI 显示 `http://本机IP:端口` | ✅ |
| 3.7 | 简易鉴权 | `data/lan_access.token` + Header / Query | ✅ |
| 3.8 | `app/web/server.py` | GUI「手机访问」开关启停 uvicorn | ✅ |
| 3.9 | 防火墙提示 | GUI 文案提示专用网络入站 | ✅ |
| 3.10 | `app/job_status.py` | Web 轮询用任务状态与日志环形缓冲 | ✅ |
...
| 4.3 | `GET /api/login/status` | 供 H5 查询 `weibo_auth.json` 是否存在 | ✅ |
...
| 里程碑 | 范围 | 可交付 |
|--------|------|--------|
| **M1** | 阶段 1 + 阶段 2 | 开发机 GUI 全流程（**已验收跑通**） |
| **M2** | 阶段 3 + 4 + 5.2 | 手机局域网 H5（**代码已就绪，待本机 + 手机联调验收**） |
| **M3** | 阶段 6 | Windows 安装包 |
